...
module Typing (Param:sig
    ...
    let process_global env vinfo = 
        Gamma.set_taint env vinfo (G [vinfo]);
        env
    (*-vinfo.vod*)
    let process_function_return env vinfo taint =
        ignore(vinfo.vid = (-vinfo.vid));
        Gamma.set_taint env vinfo taint;
        env
    
    let get_function_return_vid vinfo =
        - vinfo.vid

    let get_function_return_vinfo vinfo =
        -vinfo
end
