...
(* The environment is a mapping between symbol varinfos and taint values. *)
type environment = bool * ((varinfo, taintValue) Hashtbl.t)
...
module Gamma = struct
    ...
    let get_taint env vinfo =
        ...
        Hashtbl.find env vinfo
            
    let set_taint env vinfo taint =
        let env = match env with (_, _env) -> _env in
        (try 
            ignore (Hashtbl.find env vinfo);
            Hashtbl.remove env vinfo
        with Not_found ->
            ignore ());
        Hashtbl.add env vinfo taint
    ...
    let get_differences env env_list =
        match List.length env_list with
            ...
            let do_get_differences _env _old =
                Hashtbl.iter
                    (fun vinfo taint ->
                        let old_taint = get_taint _old vinfo in
                        match compare_taint taint old_taint with
                            | true -> ignore ()
                            | false ->
                                try 
                                    ignore(get_taint result_env vinfo)
                                with Not_found ->
                                    set_taint result_env vinfo taint)
                    _env 
            in
    ...
    let get_possible_tainted_count (_, env) =
        let count = ref 0 in
        Hashtbl.iter
            (fun vinfo taint -> if compare_taint U taint = false then count := !count + 1)
            env;
        !count 
    ...
    let pretty_print fmt env =
        ...
        Hashtbl.iter 
            (fun varinfo taint ->
                ...
                Format.fprintf fmt "\tSymname: %s = " varinfo.vname;
