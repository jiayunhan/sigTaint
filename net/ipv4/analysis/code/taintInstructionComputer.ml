...
module InstrComputer(Param:sig
    ...
    let create_initial_env_main func = 
        ...
        (List.iter
            (fun formal ->
                ignore (Gamma.set_taint initial_env formal T))
            func.sformals);  
        (List.iter 
            (fun local ->
                ignore (Typing.process_local initial_env local))
    ...
    let rec do_get_function_call_taint env vinfo param_exprs func_envs =
        ...
        let instantiate_call_taint env callee_env actuals formals vinfo =
            match Gamma.get_taint callee_env vinfo with
                | T -> T
                | U -> U
                | (G g) ->
                    List.fold_left
                        (fun t dep ->
                            ...
                            with Failure _ ->
                                ...
                                Typing.combine_taint t (Gamma.get_taint env dep))
                        U
                        g
        in
                    ...
                    if idx >= p_exp_length then
                        ...
                        if TG.is_return_param formal.vtype == false then idx + 1
	                        ...
	                        (match actual_param with
	                            | None -> ignore ()
	                            | Some a_vinfo ->
                                    let a_taint = instantiate_call_taint env callee_env param_exprs formals formal in
                                    Gamma.set_taint env a_vinfo a_taint);
                            idx + 1)
                0
                formals)
        in
        ...
        let do_globals func callee_env =
            ...
            List.iter
                (fun global ->
                    let g_taint = instantiate_call_taint env callee_env param_exprs formals global in
                    Gamma.set_taint env global g_taint)
                (list_global_vars () ())
        in
        ...
        let do_return func callee_env =
            ...
            let ret_taint = instantiate_call_taint env 
                                ...
                                param_exprs 
                                formals 
                                (Typing.get_function_return_vinfo func.svar) in
            ret_taint
        in
    ...
    let do_assign env lvalue expr cond_taint param_exprs func_env =
        let do_assign_lvalue_tainted vinfo offset =
            ...
            List.iter 
                (fun info -> Gamma.set_taint env info T)
                aliases;
            env
        in
    ...
    let do_return_instr env func null_expr cond_taint func_envs =
        ...
        (*aaa*)
        Gamma.set_taint env (Typing.get_function_return_vinfo func.svar) new_taint;
